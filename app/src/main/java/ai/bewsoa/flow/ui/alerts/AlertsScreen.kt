...
private val historyStamp = DateTimeFormatter.ofPattern("EEE d MMM · HH:mm", Locale.US)
...
fun AlertsScreen(
    ...
    Column(
        ...
        GlowCard {
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                Column(Modifier.weight(1f)) {
                    ...
                    Text(
                        "Tied to the block you're in right now — or the streak once the day is logged.",
                        style = MaterialTheme.typography.bodySmall,
                        color = TextDim
                    )
                }
        ...
        if (todayReminders.isNotEmpty()) {
            GlowCard {
                SectionHeader("Today's reminders")
                Spacer(Modifier.height(8.dp))
                todayReminders.forEach { reminder -> ReminderRow(reminder) }
            }
        }
        ...
        GlowCard {
            ...
            if (history.isEmpty()) {
                Text(
                    "Nothing sent yet. Everything the app sends will show up here.",
                    style = MaterialTheme.typography.bodySmall,
                    color = TextDim
                )
            } else {
...
@Composable
private fun ReminderRow(reminder: UpcomingReminder) {
    val now = LocalTime.now()
    val (status, color) = when {
        reminder.done -> "done ✓" to Mint
        reminder.fireAt <= now -> "sent" to TextDim
        else -> "at ${formatTime(reminder.fireAt)}" to Cyan
    }
    Row(
        ...
        Text(
            "${reminder.block.track.emoji} ${reminder.block.title}",
            style = MaterialTheme.typography.bodyMedium,
            color = if (reminder.done) TextDim else TextBright,
...
@Composable
private fun HistoryRow(entry: NotificationLogEntity) {
    Column(Modifier.padding(vertical = 6.dp)) {
        Text(
            ...
            color = if (entry.kind == "task") Cyan else Violet
        )
